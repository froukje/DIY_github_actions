"""pytorch model to predict water quality"""
...
class WaterNet(nn.Module):
    ...
    input to first layer are the 9 features from the table
...
def train(model, device, train_dataloader, optimizer, criterion, epoch, print_every):
    ...
    for batch_idx, (x, y) in enumerate(train_dataloader):
        ...
        if batch_idx % print_every == 0:
            percent = 100.0 * batch_idx / len(train_dataloader)
            print(
                f"Train Epoch {epoch} \
...
def valid(model, device, val_dataloader, criterion):
    ...
    # dropout-layers, which are ignored in this mode and batchnorm-layers,
    # which use running statistics
    model.eval()

    # disable gradient calculation
    # this is useful for inference, when we are sure not to call Tensor.backward().
    ...
    with torch.no_grad():
        ...
        for x, y in val_dataloader:
            ...
            correct += (
                (y_pred == y).sum().item()
            )  # y_pred.eq(y.view_as(y_pred)).sum().item()
            TP += torch.logical_and(y_pred == 1, y == 1).sum()
            FP += torch.logical_and(y_pred == 1, y == 0).sum()
            FN += torch.logical_and(y_pred == 0, y == 1).sum()
        ...
        print(
            f"Validation: Average loss: {val_loss.item():.4f}, \
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_accuracy, train_loss, train_recall, train_precision = train(
            model, device, train_dataloader, optimizer, criterion, epoch, print_every
        )
        ...
        predictions, val_accuracy, val_loss, val_recall, val_precision = valid(
            model, device, val_dataloader, criterion
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, default="../medium/intro_pytorch/data/water_potability.csv"
    )
